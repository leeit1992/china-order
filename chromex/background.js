// ==UserScript==
// @name                VanChuyenChina.com
// @namespace	        http://www.VanChuyenChina.com
// @description	        VanChuyenChina.com - Hệ thống Nhập hàng Trung Quốc tự động tiện dụng và hiệu quả
...
// @include		https://world.taobao.com/*
// @include		http://world.taobao.com/*
// @include		http://item.tmall.com/*
// @include		https://item.tmall.com/*
// @include		http://tw.taobao.com/*
// @include		http://detail.tmall.com/*
// @include		http://taiwan.tmall.com/*
...
/*function getHtml(price_result, boxComment){
    var s = '<div style="width:400px; padding:10px; background:#f7f7f7; border:3px solid #aaa;">';
    s +='   <div style="font-size:130%; font-weight:bold; margin-bottom:10px;" id="hangnhap-price">'+(price_result)+' VNĐ</div>';
    if(boxComment){
        s +='   <div> <label>Mô tả</label>';
        s +='         <textarea style="width:380px; height:70px; border:1px solid #ccc;" id="txtBz" name="txtBz"></textarea>';
        s +='    </div>';
    }
    s +='    <div id="block_button_cnporder"><input id="id_cnporder_add_cart"  type="button" value="Đặt hàng" style="background:#278900;border:none; padding:5px 10px; color:#fff; text-transform:uppercase; margin-top:5px;-moz-border-radius:3px; -webkit-border-radius:3px; border-radius:3px;" /></div>';
    s +='</div>' ;
    return s;
} */

function getHtml(price_result, boxComment){
    var s = '<div style="width:450px; padding:10px; background:#fbc666; border:1px solid #3F1FA5;">';
    s +='   <div style="font-size:130%; font-weight:bold; margin-bottom:10px;" id="hangnhap-price"> ~ '+(price_result)+' VND (tạm tính)</div>';
    if(boxComment){
        s +='   <div> <label>Mô tả chi tiết sản phẩm hoặc Ghi chú - Project 8</label>';
        s +='         <textarea style="width:400px; height:80px; border:1px solid #ccc;" id="txtBz" name="txtBz"></textarea>';
        s +='    </div>';
    }
    s +='    <div id="block_button_cnporder"><input id="id_cnporder_add_cart"  type="button" value="Đặt hàng" style="background:#f9050f;border:none; padding:5px 10px; color:#fff; text-transform:uppercase; margin-top:5px;-moz-border-radius:3px; -webkit-border-radius:3px; border-radius:3px;" /></div>';
    s +='</div>' ;
    return s;
}
...
function showError(mess){
    ...
    s +='   <a style="float:right; width:15px; height: 15px; border:1px solid #DDD; margin-right:2px;background:url(\'http://VanChuyenChina.com/member/themes/default/image/icon.png\') no-repeat 3px -34px transparent;" id="box-nh-box-close" href="javascript:void(0);" onclick="document.getElementById(\'box-confirm-order\').parentNode.removeChild(document.getElementById(\'box-confirm-order\'));var boxOverlay = document.getElementById(\'box_overlay\');if(boxOverlay != null) {boxOverlay.parentNode.removeChild(boxOverlay);}" data-spm-anchor-id="a220o.1000855.0.0"></a>';
    s +='    <div style="background:url(\'http://VanChuyenChina.com/member/themes/default/image/icon-error.png\') no-repeat 0 0; padding-left:45px;">';
    s +='       <h4 style="margin:0 0 10px 0; text-transform:uppercase; color:#eb0000">'+mess+'</h4>';
...
function addIframe() {
    ...
    iframe.height = 0;
    iframe.width = 0;
    iframe.src = 'https://localhost:3000/project8';
...
function taobao(cart_url,url_save) {
    ...
    function getPriceTaobao() {
        ...
        if(promotion_price != null) { // Promotion price
            if(window.location.href.indexOf('tmall') > -1 || window.location.href.indexOf('yao.95095.com') > -1) {
    ...
    function getTitle()
    {
        ...
        if(document.getElementsByClassName("tb-title").length > 0)
        {
            var h = document.getElementsByClassName("tb-title");
            tb = (h.length > 0 ? (h[0].getElementsByTagName('h3').length > 0 ? h[0].getElementsByTagName('h3')[0].textContent : ''): '');
        }
    ...
    function getSellerName()
    {
        ...
        if(shop_card.length > 0){
            ...
            if(seller_name) return encodeURIComponent(seller_name);
        }
    ...
    function getLink()
    {
        ...
        try {
            ...
            if (item_id == 0 || item_id == null || item_id == '') {
                element = document.getElementById("LineZing");
                if (element) {
                    item_id = element.getAttribute('itemid');
                } else item_id = 0;
            }
            var seller_id = '';
            ...
            if (selected_props.length > 0) {
                for (var i = 0; i < selected_props.length; i++) {
                    ...
                    if(!prop_str){
                        ...
                        if(selectedA) prop_str = selectedA[0].getAttribute("title");
                    }
            ...
            if (title.length > 0) params += '&item_title=' + title;
            if (img.length > 0) params += '&item_image=' + img;
            ...
            if(scripts.length > 0) {
                for(var script = 0; script < scripts.length; script++) {
                    if(scripts[script].innerHTML.match(/Hub\.config\.set/) || scripts[script].innerHTML.match(/TShop\.setConfig/) ) {
                        var skuMap = scripts[script].innerHTML.replace(/\s/g, '').substr(scripts[script].innerHTML.replace(/\s/g, '').indexOf(data_value) , 200);
                        if(skuMap.indexOf('skuId') > -1) {
                            params += '&outer_id=' + skuMap.substr(skuMap.indexOf('skuId') + 7, 18).match(/[0-9]+/);
                        }
                    }
                }
            }
    ...
    function linkHover() {
        var href=getLink();
        document.getElementById("id_cnporder_add_cart").setAttribute('href',href);
    }
    function xmlhttpPost(strURL) {
        ...
        var beforeHtml= document.getElementById("block_button_cnporder").innerHTML;
        document.getElementById("block_button_cnporder").innerHTML =
            '<img style="margin-top:12px;margin-right:50px" src="http://VanChuyenChina.com/member/themes/default/image/ajax-loader.gif" alt="" />';
...
//                    else console.log(self.xmlHttpReq.responseText);
                ...
                document.getElementById("block_button_cnporder").innerHTML=beforeHtml;
                addListener(document.getElementById("id_cnporder_add_cart"), 'click', linkClick);
                //addListener(document.getElementById("id_cnporder_save_item"), 'click', linkSave);
            }
        }
    ...
    function linkClick() {
        try {
            ...
            if(props.length == 0){
                ...
                console.log(J_SKU);
                if(J_SKU)  props=J_SKU.getElementsByTagName('dl');
                console.log(props);
            }
            ...
            if(full==true)
                document.getElementById("id_cnporder_add_cart").setAttribute('target',"_blank");
            else
            {
                document.getElementById("id_cnporder_add_cart").setAttribute('target',"");
                //alert("Bạn chưa chọn đầy đủ thuộc tính sản phẩm!");
                showError('Bạn chưa chọn đầy đủ thuộc tính sản phẩm!');
                document.getElementById("id_cnporder_add_cart").setAttribute('href','javascript:void(0);');
                return ;
            }
    ...
    function wait_by_call_ajax(time) {
        ...
        var self = this;
        self.xmlHttpReq = new XMLHttpRequest();
        self.xmlHttpReq.open('POST', 'https://google.com', true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
        self.xmlHttpReq.onreadystatechange = function() {
            if (self.xmlHttpReq.readyState == 4) {
    ...
    function update_html() {
        ...
        var s = getHtml(price_result, true);

        var div = document.createElement('div');
        div.innerHTML = s;
        console.log(document.getElementById('J_StrPriceModBox') );
        if(document.getElementById('J_StrPriceModBox') != null)
            ...
            document.getElementById('J_PromoWrap').parentNode.insertBefore( div.firstChild , document.getElementById('J_PromoWrap').nextSibling );
        // add cac su kien
        // addListener(document.getElementById("id_cnporder_add_cart"), 'mouseover', linkHover);
        addListener(document.getElementById("id_cnporder_add_cart"), 'click', linkClick);
        //showError('test') ;
    }
}
...
function alibaba(cart_url,url_save) {
    ...
    function getPrice(quantity) {
        ...
        var span_price = document.getElementsByClassName('mod-detail-zprice');
        // Một mức giá
        if(span_price.length > 0)
        {
            //price=span_price.textContent;
            price = span_price[0].getElementsByClassName('price')[0].textContent;
        ...
        var span_price = document.getElementsByClassName('price-original-sku');
        if(span_price.length > 0)
        {
            price = document.getElementsByClassName('price-original-sku')[0].getElementsByTagName('span')[1].textContent;
            return processPrice(price);
        }

        var span_price = document.getElementsByClassName('mod-detail-centralized-price');
        if(span_price.length > 0)
        {
            price = document.getElementsByClassName('obj-price')[0].getElementsByTagName('span')[1].textContent;
            return processPrice(price);
        }
...
//            }
//        }
        var html_prices = div_prices.getElementsByClassName("price");
        if(html_prices.length > 0) {

            var span_prices = html_prices[0].getElementsByTagName("td");
        }else
            var span_prices = div_prices.getElementsByTagName("tr");
        for (var i = 0; i < span_prices.length; i++) {
            var str = span_prices[i].getAttribute("data-range");;
            if(str){
                var json = JSON.parse(str);
                price = json.price;
                if(quantity <= json.end) {
                    break;
                }
            }
        }
    ...
    function getSellerId() {
        ...
        try{
            ...
            if(seller_id == ''){
                element = document.getElementsByClassName('contactSeller');
                if(element.length > 0) {
                    seller_id = element[0].getElementsByTagName('a')[0].innerHTML;
    ...
    function xmlhttpPost(strURL, item_data, pos) {
        ...
        document.getElementById("block_button_cnporder").innerHTML
            = '<img style="margin-top:12px;margin-right:50px" src="http://localhost:3000/project8/public/frontend/img/ajax-loader.gif" alt="" />';
        self.xmlHttpReq.open('POST', strURL, true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
...
//                    else console.log(self.xmlHttpReq.responseText);
                ...
                document.getElementById("block_button_cnporder").innerHTML = beforeHtml;
                document.getElementById("id_cnporder_add_cart").addEventListener('click', linkClick);
            }
        }
        // Send data
    ...
    function get_item_data() {
        ...
        try {
            // Multi buy
            var content = document.getElementsByClassName('mod-detail-purchasing-multiple');
            if(content.length > 0){ // New 22/5/2013
                content = content[0];
                ...
                if(input_data.length > 0) {
                     ...
                     */
                    var color = content.getElementsByClassName('leading');
                    if(color.length <= 0){
                        var color = content.getElementsByClassName('list-leading');
                    }
                    if(color.length > 0) { // Has color, and size
                        ...
                        //console.log(color);
                        var result = new Array();
                        var span_sku = content.getElementsByTagName("tbody");
                        var span_sku = span_sku[0].getElementsByTagName("tr");
                        for (var k = 0; k < span_sku.length; k++) {


                            var str = span_sku[k].getAttribute("data-sku-config");;
                            //console.log(span_sku[k]);
                            if(str){
                                if(isNaN(input_data[k].value) || input_data[k].value == 0) {
                                    continue;
                                }
                                ...
                                result[i] = new Array();
                                // Add data to arrayn
                                result[i][0] = input_data[k].value;
                                result[i][1] = json.max;
                                result[i][2] = json.skuName;
                                result[i][3] = color;

                                result[i][4] = span_sku[k].getElementsByClassName('price')[0].getElementsByTagName('em')[0].textContent.replace(/\s+/, "");
                                //console.log('Giá '+result[i][4] );
                                i++;
                            }
                        }
...
//                    }
    ...
    function getLink( item_data, pos ) {
        ...
        var lengPost = get_item_data().length;
        if(item_data[4]) item_price = item_data[4];
        ...
        if(color_size_name != '')
            params += '&color_size=' + encodeURIComponent(color_size_name);

        // Number post send
        params += '&pos=' + pos;
        params += '&length_pos=' + lengPost;
        return addVersion(params);
    }
    ...
    function linkClick() {
        ...
        for(var o in data) {
            try {
                if(data[0]['amount'] == 0) {
                    showError('Bạn chưa chọn số lượng');
                    return;
                }
                // Check min amount bought
                if(parseInt(data[0]['amount']) < parseInt(min_amount)) {
                    showError('Số lượng tối thiểu không đủ!');
                    continue;
                }
    ...
    function linkSave() {
        ...
        {
            showError("Số lượng sản phẩm không được bé hơn "+min_quantity+"!");
            document.getElementById("id_orderhang_save_item").setAttribute('href','javascript:void(0);');
            return ;
        }
        ...
        {
            showError("Số lượng sản phẩm phải là bội số của "+min_quantity+"!");
            document.getElementById("id_orderhang_save_item").setAttribute('href','javascript:void(0);');
            return ;
        }
    ...
    function update_html() {
        ...
        if(document.getElementById("id_cnporder_add_cart") != null) {
            document.getElementById("id_cnporder_add_cart").addEventListener( 'click', linkClick);
            beforeHtml = document.getElementById("block_button_cnporder").innerHTML;
        }
...
function paipai(cart_url,url_save) {
    ...
    function getSellerId()
    {
        ...
        if(element.length>0)
        {
            ...
            element = document.getElementsByName('sellerUin');
            if(element.length>0)
            {
                element=element[0];
                var seller_id=element.value;
    ...
    function xmlhttpPost(strURL) {
        ...
        var beforeHtml= document.getElementById("block_button_cnporder").innerHTML;
        document.getElementById("block_button_cnporder").innerHTML='<img style="margin-top:12px;margin-right:50px" src="http://VanChuyenChina.com/member/themes/default/image/ajax-loader.gif" alt="" />';
        self.xmlHttpReq.open('POST', strURL, true);
        self.xmlHttpReq.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
        self.xmlHttpReq.withCredentials = "true";
...
//                    else console.log(self.xmlHttpReq.responseText);
                ...
                document.getElementById("block_button_cnporder").innerHTML=beforeHtml;
                document.getElementById("id_cnporder_add_cart").addEventListener('click', linkClick);
                //document.getElementById("id_cnporder_save_item").addEventListener('click', linkSave);
            }
        }
        //console.log(getLink());
        self.xmlHttpReq.send(getLink());
    ...
    function getLink()
    {
        ...
        if(color_size_name!='')
            params+='&color_size='+color_size_name;
    ...
    function linkSave()
    {
        ...
        {
            showError("Bạn chưa chọn đầy đủ thuộc tính sản phẩm!");
            document.getElementById("id_cnporder_save_item").setAttribute('href','javascript:void(0);');
            return ;
        }
    ...
    this.htmlOnLoad=function ()
    {
        ...
        document.getElementById("id_cnporder_add_cart").addEventListener( 'mouseover', linkHover);
        document.getElementById("id_cnporder_add_cart").addEventListener( 'click', linkClick);
        //document.getElementById("id_cnporder_save_item").addEventListener( 'click', linkSave);
    };
}
function mofayichu(cart_url,url_save) {
    ...
    function linkClick()
    {
        ...
        if(parseInt(quantity)==0)
        {
            showError("Bạn phải mua ít nhất một sản phẩm!");
            document.getElementById("id_cnporder_add_cart").setAttribute('href','javascript:void(0);');
            return ;
        }
    ...
    this.htmlOnLoad=function ()
    {
        ...
        var s = getHtml(price_result, false);
...
function jj_fashion() {
    ...
    this.htmlOnLoad=function ()
    {
        ...
        var s = '<div style="border: 2px solid blue; height: 50px; margin-top: 2px; padding: 5px;">'
            ...
            + '          <br/><a href="http://VanChuyenChina.com/member/" title="trang chủ" target="_blank" style="color: blue !important; float: right; font-size: 13px; position: relative; top: 10px;">'
            + '              http://VanChuyenChina.com/member/'
            + '          </a>'
            + '      </h2>'
            + '  </div>';
...
function _1925(cart_url,url_save) {
    ...
    this.htmlOnLoad = function () {
        //var href=getLink();
        try {
            var src = 'http://VanChuyenChina.com/member/cnporder_order.gif';
            var price_taobao = getPrice();
            var price_result = getVNDPrice(price_taobao);
...
function hongwufushi(cart_url) {
    ...
    // hàm lấy số lượng mua
    function getQuantity() {
        var _quanlity = document.getElementById('cnporder_tool_amount').value;
...
if(window.location.href.indexOf('https:') > -1){
    var url = "https://localhost:3000/project8/user-tool/addToCartHttp";
}
else{
    var url = "http://VanChuyenChina.com/member/XMLHTTP.php?request=addToCart";
}
// var url = "http://192.168.1.100/checkout/cnporder/public_html/cart_taobao?";
// var url = "http://localhost/tp_gm/ajax.php?path=view&fnc=fnc_cart&act=addon_cart"; // Giangminh
var url_save = "http://VanChuyenChina.com/member/XMLHTTP.php?request=addToCollection";
